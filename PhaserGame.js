...
function preload(){
	...
	this.load.image('spBomb','assets/spbomb.png');
...
function collectSpBomb(player, spBomb){
	spBomb.disableBody(true,true);
	score += 100;
	scoreText.setText('score: '+score);
	if(spBomb.countActive(true)===0){
		spBomb.children.iterate(function(child){
			child.enableBody(true,child.x,0, true, true);
		});
